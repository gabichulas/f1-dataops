...
def upload_to_gcs(
    year: int, round: int, session_type: str, local_file_name: str, local_file_path: str
):
    ...
    destination_blob_name = (
        f"year={year}/round={round}/session_type={session_type}/{local_file_name}"
    )
    blob = bucket.blob(destination_blob_name)

    logging.info(
        f"Uploading {local_file_path} to gs://{BUCKET_NAME}/{destination_blob_name}..."
    )
...
def main():
    ...
    parser.add_argument(
        "round",
        type=int,
        help="Number of round. Example: 1 (Australia GP if you chose year=2025)",
    )
    parser.add_argument(
        "session_type",
        type=str,
        choices=["FP1", "FP2", "FP3", "Q", "S", "SS", "SQ", "R"],
        help="Type of the session. Example: 'Q' (Qualifying)",
    )
    ...
    full_path = os.path.join(
        base_path,
        f"year={args.year}/round={args.round}/session_type={args.session_type}/",
    )
    ...
    logging.info(
        f"Fetching session data for {args.year} Round {args.round} ({args.session_type})"
    )
    ...
    if BUCKET_NAME:
        try:
            ...
            upload_to_gcs(
                args.year, args.round, args.session_type, file_name, export_path
            )
        except Exception as e:
        ...
        logging.warning(
            f"BUCKET_NAME is None, no file was uploaded to GCS. The file was saved locally in {export_path}"
        )
        sys.exit(0)
